...
def test_dates_are_not_attribute_overrides():
    # created/modified map to Report metadata, not attributes.
    assert normalize_attr_name('created') == 'Created'
    assert normalize_attr_name('modified') == 'Modified'
