...
class App(JobApp):
    ...
    def run(self):
        ...
        batch_runs = math.ceil(len(self.records) / 10000)
    ...
    def _load_fast_flux_hosts(self) -> None:
        ...
        self.records = normalize_fusion_records_payload(parsed)
        if not self.records:
            self.tcex.log.error(
    ...
    def _process_records(self, start_index: int) -> None:
        """Process the records from the Fusion Fast Flux file."""
        end_index = start_index * 10000 if start_index else 10000
        if end_index > len(self.records):
